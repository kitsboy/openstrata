  ...
  <footer class="border-t border-border bg-surface-2">
    <div class="mx-auto max-w-7xl px-6 py-14">
      <div class="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div><h3 class="text-sm font-bold text-slate-800 uppercase tracking-wide mb-4">Jurisdictions</h3><ul class="space-y-2.5">{#each jurisdictions as j}<li class="flex items-center gap-2 text-sm {j.active ? 'text-slate-600' : 'text-slate-400'}"><span>{j.flag}</span><span>{j.name}</span>{#if j.active}<span class="rounded-full bg-success/10 px-2 py-0.5 text-[10px] font-bold text-success uppercase">Live</span>{:else}<span class="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold text-slate-400 uppercase">Soon</span>{/if}</li>{/each}</ul></div>
        <div><h3 class="text-sm font-bold text-slate-800 uppercase tracking-wide mb-4">{$copy.resources}</h3><div class="space-y-4"><JobsDropdown /><button class="w-full flex items-center justify-center gap-2 rounded-lg border-2 border-bitcoin/30 bg-gradient-to-r from-amber-50 to-orange-50 px-4 py-3 text-sm font-semibold text-bitcoin hover:border-bitcoin/50 transition-all" onclick={() => (donateOpen = true)}><Icon name="bitcoin" class="h-4 w-4" /><Icon name="lightning" class="h-4 w-4" /> Bitcoin & Lightning Donations</button><a href="mailto:hello@giveabit.io" class="flex items-center gap-2 text-sm text-slate-500 no-underline hover:text-brand-600 transition-colors"><Icon name="mail" class="h-4 w-4" /> hello@giveabit.io</a></div></div>
      </div>
